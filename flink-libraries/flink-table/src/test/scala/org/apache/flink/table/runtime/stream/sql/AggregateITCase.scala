...
class AggregateITCase(
  ...
  def testFirstLastWithOrder(): Unit = {
    ...
    val t1 = tEnv.sqlQuery(
      """
        |SELECT first_value(u), first_value(u, lo) as f, last_value(lo), last_value(u),
        |  last_value(u, lo) as l, i, count(lv), sum(lv), max(lv), count(fv), sum(fv), max(fv)
        |FROM (
    ...
    val sink = new TestingRetractSink
    t1.select('f, 'l).toRetractStream[Row].addSink(sink).setParallelism(1)
    env.execute()
  ...
  def testConcatAggWithNullData(): Unit = {
    ...
    val streamTable = t.toTable(tEnv, 'id, 'len, 'content, 'proc.proctime)
...
>>>>>>> upstream/blink
  def testMax2nd(): Unit = {
    ...
    val expected = List("Hello world,18,16", "Hello,9,8", "Hi,9,8")
    assertEquals(expected.sorted, sink.getRetractResults.sorted)
  }

  /** test concatAGG count **/
  @Test
  def testConcatAgg(): Unit = {
    tEnv.registerFunction("concat_agg_retract", new ConcatAggFunction)
    tEnv.registerFunction("concat_agg_ws_retract", new ConcatWsAggFunction)
    val sqlQuery =
      s"""
         |SELECT
  ...
  def testHiveUDAFSum(): Unit = {
    ...
    tEnv.registerFunction(
      ...
      new HiveUDAFFunction(
        new HiveFunctionWrapper[AbstractGenericUDAFResolver](
          classOf[GenericUDAFSum])))

    val sink = new TestingRetractSink
    tEnv
        .sqlQuery("SELECT func(s) FROM MyTable group by id")
        .toRetractStream[Row]
        .addSink(sink).setParallelism(1)
    env.execute()
    // 23=1L + 22L, 33=33L
    val expected = List("0|23", "0|33")
    assertEquals(expected.sorted, sink.getRetractResults.sorted)
  }

  @Test
  def testHiveUDAFCount(): Unit = {
    ...
    tEnv.registerFunction(
      "func",
      new HiveUDAFFunction(
        new HiveFunctionWrapper[GenericUDAFResolver2](
          classOf[GenericUDAFCount])))
